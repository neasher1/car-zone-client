...
const AllSellers = () => {
    ...
    const { data: allUsers = [], refetch } = useQuery({
        queryKey: ['allUsers'],
        queryFn: async () => {
            const res = await fetch('https://car-zone-server.vercel.app/allseller', {
                headers: {
    ...
    const handleDelete = (id) => {
        console.log(id);
        fetch(`https://car-zone-server.vercel.app/allseller/${id}`, {
            method: 'DELETE'
        })
            .then(res => res.json())
            .then(data => {
    ...
    const handleVerify = (id) => {
        fetch(`https://car-zone-server.vercel.app/verifyseller/${id}`, {
            method: 'PUT'
        })
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <div>
            <div className='card shadow-2xl p-8 m-4 w-full'>
                <h3 className="text-2xl mb-4">All Sellers</h3>
